    ...
    if ($global:DefaultVIServer.Version -eq "6.7.0") {
        ...
        if ($null -eq $timeSync) {
            ...
            if ($timeSyncMode -eq "NTP") {
                [string]$NTPServersList = [string]::join("<br/>",$ntpServers.split(" "))
                $NTPStatus = "N/A"
            } else {
                $NTPServersList = "N/A"
                $NTPStatus = "N/A"
        ...
        if ($null -eq $timeSync) {
            ...
            if ($timeSyncMode -eq "NTP") {
                $NTPServersList = [string]::join("<br/>",$ntpServers.servers.split(" "))
                $NTPStatus = $ntpServers.status
            } else {
                $NTPServersList = "N/A"
                $NTPStatus = "N/A"
            }
        ...
        # 6.0 and before cannot return anything so let's set it to N/A
        $timeSyncMode = "N/A"
    }
...
if ($VCSA) {
    ...
    $TableFormat =	@{"Mode" = @(@{ "-eq `"DISABLED`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"NTPServers" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"NTPStatus" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; }, @{"-eq `"down`"" = "Cell,cid|red|16x16"; })}

}

$Title = "[vCenter] VCSA Time"
$Header = "[vCenter] VCSA Time settings"
$Comments = "Shows the VCSA's Time settings. Gray = Disabled; Red = Down (NTP Servers)"
$Display = "Table"
