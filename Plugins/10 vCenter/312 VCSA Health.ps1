# Start of Settings
# End of Settings

Function Get-VAMIHealth {
    <#
    .NOTES
    ===========================================================================
     Created by:    William Lam, Dario Doerflinger
	===========================================================================
    .SYNOPSIS
        This function retrieves health information from VAMI interface (5480)
        for a VCSA node which can be an Embedded VCSA, External PSC or External VCSA.
    .DESCRIPTION
        Function to return VAMI health
    .EXAMPLE
        Connect-CisServer -Server 192.168.1.51 -User administrator@vsphere.local -Password VMware1!
        Get-VAMIHealth
#>
    
    #<# Prod Block
    $healthOverall = (Get-CisService -Name 'com.vmware.appliance.health.system').get()
    $healthLastCheck = (Get-CisService -Name 'com.vmware.appliance.health.system').lastcheck()
    $healthCPU = (Get-CisService -Name 'com.vmware.appliance.health.load').get()
    $healthMem = (Get-CisService -Name 'com.vmware.appliance.health.mem').get()
    $healthSwap = (Get-CisService -Name 'com.vmware.appliance.health.swap').get()
    $healthStorage = (Get-CisService -Name 'com.vmware.appliance.health.storage').get()
    $healthSoftwareUpdates = (Get-CisService -Name 'com.vmware.appliance.health.softwarepackages').get()
    ##>
    
    <# Testing Block
    $healthLastCheck = (Get-CisService -Name 'com.vmware.appliance.health.system').lastcheck()
    $healthSoftwareUpdates = "red"
    $healthOverall = "green"
    $healthCPU = "yellow"
    $healthMem = "orange"
    $healthSwap = "gray"
    $healthStorage = "green"
    ##>

    # applmgmt only exists in 6.7.0 version or later
    if ($global:DefaultVIServer.Version -eq "6.7.0") {
        $healthApplmgmt = (Get-CisService -Name 'com.vmware.appliance.health.applmgmt').get()

    } else {
        $healthApplmgmt = "gray"
    }



    # DB health only applicable for Embedded/External VCSA Node
    $vami = (Get-CisService -Name 'com.vmware.appliance.system.version').get()

    if ($vami.type -eq "vCenter Server with an embedded Platform Services Controller" -or $vami.type -eq "vCenter Server with an external Platform Services Controller") {
        $healthVCDB = (Get-CisService -Name 'com.vmware.appliance.health.databasestorage').get()
    }
    else {
        $healthVCDB = "gray"
    }
    

    $healthResult = [pscustomobject] @{
        HealthOverall   = $healthOverall;
        HealthLastCheck = $healthLastCheck;
        HealthCPU       = $healthCPU;
        HealthMem       = $healthMem;
        HealthSwap      = $healthSwap;
        HealthStorage   = $healthStorage;
        HealthVCDB      = $healthVCDB;
        HealthSoftware  = $healthSoftwareUpdates;
        Applmgmt        = $healthApplmgmt 
    }
    $healthResult
}

if ($VCSA) {
    Get-VAMIHealth
   

    Add-ReportResource -cid "red" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "green" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "gray" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "yellow" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "orange" -Type "base64" -ResourceData "png|"
    
    $TableFormat =	@{"HealthOverall" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthCPU" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthMem" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthSwap" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthStorage" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthVCDB" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"HealthSoftware" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
    $TableFormat +=	@{"Applmgmt" = @(@{ "-eq `"green`"" = "Cell,cid|green|16x16"; }, @{ "-eq `"red`"" = "Cell,cid|red|16x16" }, @{ "-eq `"gray`"" = "Cell,cid|gray|16x16" }, @{ "-eq `"yellow`"" = "Cell,cid|yellow|16x16" }, @{ "-eq `"orange`"" = "Cell,cid|orange|16x16" })}
}

$Title = "[vCenter] VCSA Health"
$Header = "[vCenter] VCSA Health status"
$Comments = "Green = Good; Yellow = Problems?; Orange = Degraded; Red = Unavailable; Gray = No Data"
$Display = "Table"
$Author = "Dario Doerflinger"
$PluginVersion = 1.0
$PluginCategory = "vSphere"
