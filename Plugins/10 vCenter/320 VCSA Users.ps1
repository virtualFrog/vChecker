# Start of Settings
# End of Settings

Function Get-VAMIUser {
    <#
    .NOTES
    ===========================================================================
     Created by:    William Lam, Dario Doerflinger
	===========================================================================
	.SYNOPSIS
		This function retrieves VAMI local users using VAMI interface (5480)
        for a VCSA node which can be an Embedded VCSA, External PSC or External VCSA.
	.DESCRIPTION
		Function to retrieve VAMI local users
	.EXAMPLE
        Connect-CisServer -Server 192.168.1.51 -User administrator@vsphere.local -Password VMware1!
        Get-VAMIUser
#>
    if ($global:DefaultVIServer.Version -eq "6.7.0") {
        $userAPI = Get-CisService -Name 'com.vmware.appliance.local_accounts'
        $userList = $userAPI.list()
        $userResults = @()

        foreach ($userID in $userList) {
            try {
                $userObject = $userAPI.get($userID)
                #check for errors in API query answer
            }
            catch {
                Write-Host -ForegroundColor Red "Could not get User-Detail. Was this VCSA consolidated with a external PSC?"
                break
            }
            
            
                if ($null -eq $userObject.email) {
                    $userEmail = "N/A"
                }
                else {
                    $userEmail = $userObject.email
                }

                if ($null -eq $userObject.fullname) {
                    $userFullName = "N/A"
                }
                else {
                    $userFullName = $userObject.fullname
                }

                if ($null -eq $userObject.last_password_change) {
                    $userLastPWChange = "N/A"
                }
                else {
                    $userLastPWChange = $userObject.last_password_change
                }

                if ($null -eq $userObject.password_expires_at) {
                    $userPasswordExpiresAt = "N/A"
                }
                else {
                    $userPasswordExpiresAt = $userObject.password_expires_at
                }

                if ($null -eq $userObject.inactive_at) {
                    $userInactiveAt = "N/A"
                }
                else {
                    $userInactiveAt = $userObject.inactive_at
                }

                $userInfo = New-Object psobject
                $userInfo | Add-Member -MemberType NoteProperty -Name "Name" -Value $userID
                $userInfo | Add-Member -MemberType NoteProperty -Name "Full Name" -Value $userFullName
                $userInfo | Add-Member -MemberType NoteProperty -Name "Enabled" -Value $userObject.enabled
                $userInfo | Add-Member -MemberType NoteProperty -Name "Email" -Value $userEmail
                $userInfo | Add-Member -MemberType NoteProperty -Name "Password" -Value $userObject.has_password
                $userInfo | Add-Member -MemberType NoteProperty -Name "PW changed" -Value $userLastPWChange
                $userInfo | Add-Member -MemberType NoteProperty -Name "PW Expires" -Value $userPasswordExpiresAt
                $userInfo | Add-Member -MemberType NoteProperty -Name "User Expires" -Value $userInactiveAt

                $userResults += $userInfo
            
        }

    }
    elseif ($global:DefaultVIServer.Version -eq "6.5.0") {
        $userAPI = Get-CisService 'com.vmware.appliance.techpreview.localaccounts.user'
        
        $userResults = @()
        $users = $userAPI.list()
        foreach ($user in $users) {
            $userString = [pscustomobject] @{
                User           = $user.username
                Name           = $user.fullname
                Email          = $user.email
                Status         = $user.status
                PasswordStatus = $user.passwordstatus
                Role           = $user.role
            }
            $userResults += $userString
        }
    }
    else {
        # Nope
        $failure = "This plugin is not compatible with version $($global:DefaultVIServer.Version) at this time"
        $failure
        exit
    }

    
    $userResults
}


if ($VCSA) {
    Get-VAMIUser 
     
    #Add-ReportResource -cid "red" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "green" -Type "base64" -ResourceData "png|"
    Add-ReportResource -cid "gray" -Type "base64" -ResourceData "png|"
    #Add-ReportResource -cid "yellow" -Type "base64" -ResourceData "png|"
    #Add-ReportResource -cid "orange" -Type "base64" -ResourceData "png|"
    
    $TableFormat =	@{"Enabled" = @(@{ "-eq `$true" = "Cell,cid|green|16x16"; }, @{"-eq `$false" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"Password" = @(@{ "-eq `$true" = "Cell,cid|green|16x16"; }, @{"-eq `$false" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"Full Name" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"Email" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"PW Changed" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"PW Expires" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
    $TableFormat +=	@{"User Expires" = @(@{ "-eq `"N/A`"" = "Cell,cid|gray|16x16"; })}
}

$Title = "[vCenter] VCSA User"
$Header = "[vCenter] VCSA local Users"
$Comments = "Green = True; Grey = False"
$Display = "Table"
$Author = "Dario Doerflinger"
$PluginVersion = 1.1
$PluginCategory = "vSphere"

# Change Log:
## 1.1 : Catch Errors in API Response for 6.7 U1