...
$vms = $vmviews | Select-Object Name,Config.Version,Runtime.PowerState,Runtime.FeatureRequirement

$result = @()
foreach ($Loopvm in $vms | Sort-Object -Property Name) {
    # Only check VMs that are powered on
    if($Loopvm.Runtime.PowerState -eq "poweredOn") {
        $vmDisplayName = $Loopvm.Name
        $vmvHW = $Loopvm.Config.Version
        ...
        $cpuFeatures = $Loopvm.Runtime.FeatureRequirement
